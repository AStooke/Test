...
def build_cnn(input_shape, output_size, size="small"):
    print("Building CNN")
...
def load_data(input_shape, output_size):
    ...
    x = np.random.randn(DATA, *input_shape).astype("float32")
...
def main():
    ...
    logits, input_var, trainable_variables = build_cnn(input_shape, output_size)
    ...
    with tf.Session() as sess:
        ...
        for _ in range(5):
            r = f_inference.eval(feed_dict={input_var: x[:BATCH]})
            # r = sess.run(f_inference, feed_dict={input_var: x[:BATCH]})

        t_0 = time.time()
        for _ in range(EPOCHS):
            for i in range(BATCHES):
                r = f_inference.eval(feed_dict={input_var: x[i * BATCH:(i + 1) * BATCH]})
        ...
        for _ in range(5):
            r = f_train.run(feed_dict={input_var: x[:BATCH],
                                      target_var: y[:BATCH]})
        ...
        for _ in range(EPOCHS):
            for i in range(BATCHES):
                r = f_train.run(feed_dict={input_var: x[i * BATCH:(i + 1) * BATCH],
