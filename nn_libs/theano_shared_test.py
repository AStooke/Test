...
def build_cnn(
    ...
    input_var = theano.shared(np.random.randn(BATCH, *input_shape).astype("float32"))
...
def build_inference(output_layer, input_var):
    ...
    f_inference = theano.function(inputs=[], outputs=test_prediction)
...
def main():
    ...
    target_var = theano.shared(np.random.randint(0, 10, size=BATCH).astype("int32"))
