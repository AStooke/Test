...
class SynkVariables(struct):
    ...
    def _include(self, var):
        is_new_var = var in self.vars
        if not is_new_var:
            var_ID = self.vars.index(var)
        else:
            var_ID = self.num
            self.vars.append(var)
            self.names.appand(var.name)
    ...
    def _alloc_shmem(self, var_ID, shape, tag_ID, create):
        ...
        return shmem  # would like to put it in self.shmems but can't bc master has list of lists for shareds
